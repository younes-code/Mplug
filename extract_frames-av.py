...
if __name__ == "__main__":
    ...
    if os.path.isdir(opt.dataset_path):
        print(f"You have access to the directory: {opt.dataset_path}")
    else:
        print(f"You do not have access to the directory: {opt.dataset_path}")
    print()
...
#     print (video_paths)
    for i, video_path in enumerate(video_paths):
      #   sequence_type, sequence_name = video_path.split(".mp4")[0].split("\\")[-2:]
        sequence_type, sequence_name = os.path.split(video_path)[-2:]
